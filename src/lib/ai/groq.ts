...
export class GroqProvider implements AIProvider {
  ...
  initialize(config: AIProviderConfig): void {
    ...
    this.endpoint = config.apiEndpoint?.replace(/\/$/, "") || "https://api.groq.com/openai/v1";
  ...
  private async chat(prompt: string): Promise<{ text: string; tokensUsed?: number }> {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Groq request failed: ${response.status} ${response.statusText} ${details}`);
    }
  ...
  async summarize(text: string, maxLength = 8000): Promise<SummarizeResult> {
    try {
      ...
      throw new Error(`Groq summarize failed: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  }
  ...
  async extractTags(text: string, existingTags: string[]): Promise<ExtractTagsResult> {
    try {
      ...
      throw new Error(`Groq extractTags failed: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  }
  ...
  async formatContent(html: string): Promise<FormatContentResult> {
    try {
      ...
      throw new Error(`Groq formatContent failed: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  }
