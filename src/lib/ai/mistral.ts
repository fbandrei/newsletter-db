...
export class MistralProvider implements AIProvider {
  ...
  initialize(config: AIProviderConfig): void {
    ...
    this.endpoint = config.apiEndpoint?.replace(/\/$/, "") || "https://api.mistral.ai/v1";
  ...
  private async chat(prompt: string): Promise<{ text: string; tokensUsed?: number }> {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Mistral request failed: ${response.status} ${response.statusText} ${details}`);
    }
  ...
  async summarize(text: string, maxLength = 8000): Promise<SummarizeResult> {
    try {
      ...
      throw new Error(`Mistral summarize failed: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  }
  ...
  async extractTags(text: string, existingTags: string[]): Promise<ExtractTagsResult> {
    try {
      ...
      throw new Error(`Mistral extractTags failed: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  }
  ...
  async formatContent(html: string): Promise<FormatContentResult> {
    try {
      ...
      throw new Error(`Mistral formatContent failed: ${error instanceof Error ? error.message : "Unknown error"}`);
    }
  }
